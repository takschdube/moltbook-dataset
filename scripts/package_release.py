...
Creates:
  ...
  releases/moltbook-dataset-YYYY-MM.zip         (monthly snapshot)
...
def load_data():
    """Load all data files and compute stats."""
    files = {}
    stats = {}
    ...
    for name in data_files:
        ...
        if path.exists():
            ...
            # Specific stats
            if name == "submolts.json":
                stats["submolts"] = len(data)
            elif name == "posts.json":
                stats["posts"] = len(data)
            elif name == "posts_full.json":
                stats["posts_with_comments"] = len(data)
                stats["comments"] = count_comments(data)
            elif name == "agents.json":
                stats["agents"] = len(data)
            elif name == "social_graph.json":
                stats["interactions"] = len(data)
            elif name == "platform_stats.json":
...
def generate_release_notes(files, stats, zip_info):
    ...
    lines.append(f"| Agents (Moltys) | {stats.get('agents', 0):,} |")
    lines.append(f"| Social Interactions | {stats.get('interactions', 0):,} |")
    lines.append("")

    lines.append("## Files\n")
    lines.append("| File | Records | Original Size |")
    lines.append("|------|---------|---------------|")
    ...
    lines.append(
        f"| Archive | Compressed | Original | Ratio |"
    )
    lines.append("|---------|------------|----------|-------|")
    lines.append(
...
def main():
    ...
    for name, info in files.items():
        count = f"{info['count']:,} records" if info["count"] else "object"
